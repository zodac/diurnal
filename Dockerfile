...
# outputs in below.
FROM node:26.8.2-alpine AS css
...
FROM node:26.8.2-alpine AS icons
...
FROM node:26.8.2-trixie AS nodesrc
...
LABEL org.opencontainers.image.authors="zodac <arouge110@msn.com>" \
      ...
      org.opencontainers.image.description="Self-hosted application for tracking daily habits, with calendar views, statistics and daily notes" \
