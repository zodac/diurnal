...
- **`NO_STACKED_MARKS`** rejects a run of more than `MAX_CONSECUTIVE_MARKS` (8) consecutive non-spacing/enclosing
  marks - the "zalgo" pattern, which renders as a column of glyphs that overflows the row it is shown in. Eight
  clears every script that genuinely stacks marks (Hebrew with points and cantillation, Thai, Devanagari,
  Vietnamese) with room to spare. Note that normalisation composes the FIRST mark onto the letter before it, so a submission needs
...
| More than 8 stacked combining marks ("zalgo")                                                                                                           | **rejected**                  | Renders as a column of glyphs that overflows the row                                                                                                                                                                                                                            |
