...
- **`documentation`** — the 17 README shots in `docs/screenshots/`. **Also gitignored, and also generated by the
  build**: `publish.yml` recaptures them on every release (`DOC_SCREENSHOTS=true`) and replaces the assets on the
  standalone `readme-assets` GitHub release, which the README and `docs/dockerhub-overview.md` embed by absolute
  URL. They no longer lag. Run it by hand only to REVIEW what a change will publish: `scripts/dev-up.sh`, then
  `node scripts/generate-screenshots.cjs documentation`, then `scripts/dev-teardown.sh`. Output is WebP. To
  refresh the published set between releases: `gh release upload readme-assets --clobber docs/screenshots/*.webp`.
