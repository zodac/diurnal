...
> **A note's CONTENT must never be translated or otherwise machine-altered — only the surrounding UI.** The same
...
> not via a translation feature, not as a side effect of the [`.claude/I18N.md`](I18N.md) work, not even as an
> optional convenience. `.claude/I18N.md` documents how the app's OWN chrome (labels, buttons, messages,
> date/number formatting) is translated — a note's stored `content` is opaque user data that passes through unmodified end to
...
- **Written synchronously on every keystroke** (and on save/undo/clear), deliberately not debounced. A full quota or unavailable storage is swallowed
  **silently** — the draft then just does not outlive the page, which is the behaviour this replaced, so there is nothing to tell the user.
...
> **"No key" and "no notes" are the same state, and that is enforced rather than assumed.** Every account has minted a
> key since `user_notes_keys` was introduced, and the migration that introduced it emptied the notes table rather than
> back-filling — so an account without a key row provably holds nothing, and the write path may mint on demand for it
> safely. Since the 1.0.0 schema collapse that population is empty on any new deployment; the on-demand mint stays as
> the guard described below rather than as a legacy path.
...
| Who holds the secret              | **The deployment**                                     | The user, as a passphrase. Built, then removed: it bought protection from the operator, and cost a second secret to keep, an unlock step on every new session, and a failure mode where forgetting it destroyed years of writing with no remedy                                                                                                                                                                                   |
...
| Existing plaintext notes          | **The table was emptied and the column dropped**       | Keeping it for compatibility. Rejected: a readable column is a standing invitation, and every path that could write one had gone. There is no key at migration time to seal them with, and inventing one in SQL would mean writing it into the very database this protects                                                                                                                                                        |
...
pin — there is no plaintext `notes.content` column and the sealed `bytea` has no width — so the bound lives purely in
